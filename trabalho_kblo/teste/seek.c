...
int main ()
{
   int i, j, handle, handle2;
    ...
    if( seek2(handle2, 10) == ERROR_SIGNAL ) return 7;
    if( read2(handle2, m, 100) == ERROR_SIGNAL ) return 6;
    printf("%s\n",m);
    close2(handle);
    close2(handle2);
    return 0;
